...
class AttentionModel(nn.Module):
    ...
    def forward(self, x):
        ...
        resnet_features = self.backbone(x)  # Output from ResNet backbone
        B, C, H, W = resnet_features.size()
        x_for_additive = resnet_features.view(B, C, H * W).permute(0, 2, 1)
        global_context = self.avg_pool(resnet_features).view(B, C)

        context1, _ = self.scaled_dot_product(resnet_features, resnet_features, resnet_features)
        context2, _ = self.multiplicative_attention(resnet_features, resnet_features, resnet_features)
        ...
        top_k_resnet_features = self.top_k_channel_selection(resnet_features)
        # top_k_resnet_features = self.dropout(top_k_resnet_features)
        ...
        concatenated = torch.cat([context1, context2, context3, top_k_resnet_features], dim=1)
